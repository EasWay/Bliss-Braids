...
class InfiniteGridMenu {
  ...
  private render(): void {
    ...
    gl.clearColor(0.008, 0.004, 0.098, 0); // Match page background #020119 but transparent
...
// Convert PortfolioImage to MenuItem format
function portfolioToMenuItem(portfolioImage: PortfolioImage): MenuItem {
  return {
    image: portfolioImage.url,
    link: `/booking?style=${portfolioImage.id}&category=${portfolioImage.category}`,
    title: portfolioImage.styleName,
    description: `Starting from GHS ${portfolioImage.basePrice}`
  };
}
...
const defaultItems: MenuItem[] = [
  {
    ...
    description: 'Discover beautiful braiding styles'
  }
];
...
interface InfiniteMenuProps {
  ...
  items?: MenuItem[];
  scale?: number;
  onItemClick?: (portfolioImage: PortfolioImage) => void;
}

const InfiniteMenu: FC<InfiniteMenuProps> = ({ portfolioImages = [], items, scale = 1.0, onItemClick }) => {
  ...
  const menuItems = useMemo(() => {
    ...
    if (portfolioImages.length > 0) {
      return portfolioImages.map(portfolioToMenuItem);
    }
    return defaultItems;
  ...
  useEffect(() => {
    ...
    const handleActiveItem = (index: number) => {
      ...
      if (itemIndex !== currentActiveIndex) {
        ...
        // Also set the corresponding portfolio image if available
        if (portfolioImages.length > 0) {
          const portfolioIndex = index % portfolioImages.length;
          setActivePortfolioImage(portfolioImages[portfolioIndex]);
        }
      }
    };

    if (canvas && menuItems.length > 0) {
  ...
  }, [menuItems, scale]); // Remove portfolioImages from dependencies since it's included in menuItems

  const handleButtonClick = () => {
    if (activePortfolioImage && onItemClick) {
      onItemClick(activePortfolioImage);
